...
use sui_simulator::{DBSimulator, SimulateResult, Simulator};
...
#[tokio::main]
async fn main() {
    println!("========== DBSimulator Shift Monitor ==========");
    ...
    let simulator = DBSimulator::new(RPC_URL).await.expect("Failed to create simulator");
    ...
    // Create tracer and get violations handle before simulation
    let tracer = ShiftViolationTracer::new();
    let shift_violations_handle = tracer.shift_violations();

    let start = Instant::now();
    let result = simulator
        .simulate(tx, override_objects, Some(Box::new(tracer)))
        .await
        .expect("Simulation failed");

    println!("✅ [{:?}] Simulation completed successfully", start.elapsed());

    let shift_violations = shift_violations_handle.lock().unwrap().clone();

    let SimulateResult { events, .. } = result;

    println!("Shift Violations 🧀 {:?}", shift_violations);
